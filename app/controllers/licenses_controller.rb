...
class LicensesController < ApplicationController
  ...
  def code_details
    sql = "select count(i.id) total_count, ifnull(count(case when i.user_id is null then i.id end), 0) n_used_count,
      ...
      inner join buses b on b.id = i.bus_id inner join vicegerents v on v.id = i.vicegerent_id where i.status=#{InviteCode::STATUS[:NOMAL]} "
    sql += " v.name like '%#{params[:info].strip}%'" unless params[:info].nil? or params[:info].strip.empty?
    sql += " group by b.id"
