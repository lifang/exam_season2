...
//修改单词时，检查选择的例句数量，最少三条,一个都不选表示不修改。
function check(){
    var check_sum=0;
    var result = "";
    $("input[name='sentence_checkbox']").each(function(){
        if($(this).is(":checked")){
            check_sum += 1;
            if(result!=""){
                result+=";";
    ...
    if(check_sum==0){
        return true; //即用户没有操作例句，不作处理
    }
    if(check_sum>=3){
        $("#sentence").val(result);
        return true;
    }else{
        alert("请保留至少三条例句（提示：全都不选代表暂不修改）");//用户选择了例句，则不能少于三条
        return false;
    }
}