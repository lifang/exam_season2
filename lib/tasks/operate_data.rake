...
namespace :operate do
  ...
  task(:data => :environment) do
    ...
    num << detail.shift(4).join(",")
    detail<< Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null  ")
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null and " + TIME_EXPRESSION)
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null and " + seven_expr)
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null and " + one_expr)
    ...
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null  ")
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null  and " + TIME_EXPRESSION)
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null  and " + seven_expr)
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and category_id is not null  and " + one_expr)
