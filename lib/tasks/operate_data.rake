...
namespace :operate do
  ...
  task(:data => :environment) do
    ...
    num << detail.shift(4).join(",")
    detail<< Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} ")
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + TIME_EXPRESSION)
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + seven_expr)
    detail << Order.count(:id, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + one_expr)
    ...
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} ")
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + TIME_EXPRESSION)
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + seven_expr)
    detail << Order.sum(:total_price, :conditions =>"types = #{Order::TYPES[:CHARGE]} and " + one_expr)
